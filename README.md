...
| [Harness Design for Long-Running Application Development](./harness-design-long-running-apps/) | Planner + Generator + Evaluator 三角色、上下文焦虑、自评乐观偏差、20× 成本对比 | 中高级 |
| [Scaling Managed Agents](./managed-agents/) | 大脑/双手解耦、会话作为外部事件日志、p95 TTFT ↓90%、多大脑 × 多双手平台 | 高级 |
...
| [Claude Code Auto Mode](./claude-code-auto-mode/) | 分类器替代人工确认、三层决策、两阶段过滤（8.5% → 0.4% FPR）、17% FNR 权衡 | 中高级 |
...
│       ├── 多角色 Harness（Planner + Generator + Evaluator）
│       ├── Managed Agents 平台（大脑/双手解耦）
...
| Managed Agents 让 p95 首 token 延迟下降 **>90%** | Scaling Managed Agents |
| 自动模式两阶段分类器误报率 **8.5% → 0.4%** | Claude Code Auto Mode |
| 用户对人工确认提示通过率 **93%**（导致疲劳） | Claude Code Auto Mode |
| 复古游戏机构建：单 Agent $9 vs Harness **$200（20×）** | Harness Design Long-Running Apps |
